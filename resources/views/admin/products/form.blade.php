<x-admin.layout.layout :title="$product ? 'Edit Product' : 'Create Product'">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-4 mt-2">
                    ...
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" class="sr-only peer" {{ old('is_active',
                            $product->is_active ?? true) ? 'checked' : '' }}>
                        <div
                            class="relative w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
                        </div>
                    ...
                    {{-- Is Stock --}}
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_stock" class="sr-only peer" {{ old('is_stock',
                            $product->is_stock ?? true) ? 'checked' : '' }}>
                        <div
                            class="relative w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-4">
                        {{-- Preview --}}
                        <div
                            class="w-32 h-32 rounded border border-dashed border-gray-300 flex items-center justify-center bg-gray-100 overflow-hidden">
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <select name="variants[{{ $index }}][color_id]" class="select" required>
                                ...
                                <option value="{{ $color->id }}" @selected( (isset($variant['product_color_id']) &&
                                    $variant['product_color_id']==$color->id) ||
                                    (isset($variant['color_id']) && $variant['color_id'] == $color->id) ||
                                    old("variants.$index.color_id") == $color->id
                                    )
                                    >
                                    {{ $color->name }}
                                </option>
                        ...
                        <div>
                            ...
                            <select name="variants[{{ $index }}][size_id]" class="select">
                                ...
                                <option value="{{ $size->id }}" @selected( (isset($variant['product_size_id']) &&
                                    $variant['product_size_id']==$size->id) ||
                                    (isset($variant['size_id']) && $variant['size_id'] == $size->id) ||
                                    old("variants.$index.size_id") == $size->id
                                    )
                                    >
                                    {{ $size->name }}
                                </option>
                        ...
                        {{-- Image Upload with Preview (Variant) --}}
                        <div>
                            <label class="block mb-1 font-semibold">Image</label>
                            <input type="file" name="variants[{{ $index }}][image]" class="input variant-image-input"
                                data-preview-id="variant-preview-{{ $index }}">
                            <img id="variant-preview-{{ $index }}"
                                src="{{ !empty($variant->image) ? asset($variant->image) : 'https://via.placeholder.com/80x80?text=Preview' }}"
                                class="h-16 mt-1 rounded object-cover" />
                        </div>
                    </div>
                    @endforeach
                </div>
    ...
    <script>
        ...
        document.getElementById('add-variant').addEventListener('click', () => {
            ...
            variant.className = "p-4 bg-gray-50 border border-gray-200 rounded-md shadow-sm grid grid-cols-1 md:grid-cols-5 gap-4 items-end variant-item";
            ...
            variant.innerHTML = `
                ...
                <div>
                    <label class="block mb-1 font-semibold">Image</label>
                    <input type="file" name="variants[${index}][image]" class="input variant-image-input" data-preview-id="variant-preview-${index}">
                    <img id="variant-preview-${index}" src="https://via.placeholder.com/80x80?text=Preview" class="h-16 mt-1 rounded object-cover" />
                </div>
            `;
