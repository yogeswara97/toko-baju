<x-admin.layout.layout :title="$product ? 'Edit Product' : 'Create Product'">
    ...
    {{-- === MODAL UNTUK CROP === --}}
    <div id="cropper-modal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 hidden">
        <div class="bg-white p-4 rounded-md shadow-lg max-w-md w-full">
            <div class="mb-4 text-lg font-bold">Crop Image</div>
            <div>
                <img id="cropper-image" class="max-w-full h-auto" />
            </div>
            <div class="mt-4 flex justify-end gap-2">
                <button type="button" id="cancel-crop"
                    class="px-4 py-1 bg-gray-200 rounded hover:bg-gray-300">Cancel</button>
                <button type="button" id="apply-crop"
                    class="px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">Apply</button>
            </div>
        </div>
    </div>
    ...
    // === Trigger Crop for Variant & Main Image ===
    document.addEventListener('change', function(e) {
        if (e.target.matches('.variant-image-input') || e.target.matches('#image')) {
            const file = e.target.files[0];
            const previewId = e.target.getAttribute('data-preview-id') || 'preview';
    ...
    document.getElementById('apply-crop').addEventListener('click', () => {
        ...
        cropper.getCroppedCanvas().toBlob(blob => {
            ...
            currentPreview.onload = () => URL.revokeObjectURL(currentPreview.src);
