<x-customer.layout.layout>
    <section class="container-custom">
        <x-customer.page-header
            title="{{ $mode === 'edit' ? 'Edit Alamat' : ($mode === 'show' ? 'Detail Alamat' : 'Tambah Alamat') }}"
            description="Isi detail alamat pengirimanmu dengan akurat untuk memastikan kelancaran transaksi." />
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-8">
            ...
            <div class="lg:col-span-2">
                ...
                @if ($mode !== 'show')
                <form
                    action="{{ $mode === 'edit' ? route('customer.address.update', $address->id) : route('customer.address.store') }}"
        ...
        <div class="">
            <div class=" p-6 rounded-xl h-full">
                <h4 class="text-lg font-semibold text-gray-800 mb-4">Tips Mengatur Alamat</h4>
                <ul class="text-sm text-gray-600 space-y-2 list-disc pl-4">
                    <li>Gunakan nama yang mudah diingat, seperti "Rumah" atau "Kantor"</li>
                    <li>Pakai pencarian lokasi agar data wilayah otomatis terisi</li>
                    <li>Jadikan alamat utama jika paling sering dipakai</li>
                </ul>
            </div>
        </div>
        </div>
                ...
                try {
                    ...
                    data.forEach(item => {
                        ...
                        li.innerHTML = `<a href="#" class="block px-4 py-2 hover:bg-gray-100 text-gray-800"
                            data-id="${item.id}" data-label="${item.label}" data-subdistrict="${item.subdistrict_name}"
                            data-district="${item.district_name}" data-city="${item.city_name}"
                            data-province="${item.province_name}" data-zip="${item.zip_code}">
                            ${item.label}
                        </a>`;
